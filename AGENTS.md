...
- **It assigns only what someone would actually read.** A second comparison runs with the self-moving facts stripped out as well: `updatedAt` follows any commit in a repository, `stars` follows anyone's click, and inside `downloads` the version, URL and size rotate with every release. A pull request that changes nothing else is left unassigned and merges itself unseen. Everything left over draws an assignee — a licence, the archived flag, a platform that dropped out of `downloads`, and every editorial change, whose *parsed* shape is visible nowhere else. The platform/format pairs stay in the comparison because `build.ts` only fails when *no* asset matches the platform table: one installer quietly missing from a release would otherwise take a download button off the page unseen. Auto-merge stays on either way — what an assignee is asked is "is this right?", not "should this land?", and the answer to a wrong entry is a commit under `projects/`, never a held pull request, which the next run would re-point out from under the reader anyway.
